...
export class BatchMasterComponent implements OnInit {
  ...
  todayDate: string;
  constructor(private fb: FormBuilder, public API: ApiserviceService, private router: Router,private location: Location) {
    this.fetchData();
  ...
  getunitname() {
    ...
    this.API.getDataByUserId('UnitMaster',userid).subscribe((res) => {
      this.UnitMasterlist = res
    })

    this.API.getDataByUserid('BatchMaster',userid).subscribe((res) => {
  ...
  callEdit(id:any) {
    ...
    this.API.getsingleid('BatchMaster',id).subscribe((res)=>{
      this.tableDataList = res[0].BatchUnitData
      if(!res.startDate){
        this.batchMasterForm.controls['startDate'].setValue(this.todayDate)        
      }else{
